...
export default function ConfigSideNav() {
  ...
  const ingestData = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("http://gpt-on-premises-server.default:5000/ingest");
  ...
  const handleDownloadModel = async () => {
    try {
      ...
      const res = await fetch("http://gpt-on-premises-server.default:5000/download_model");
      const jsonData = await res.json();
      if (!res.ok) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("http://gpt-on-premises-server.default:5000/upload_doc", {
        method: "POST",
        body: formData,
      });
